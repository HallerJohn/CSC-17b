<?php 
//this page displays the list of accounts and handles editting/deleting them
...
if (isset($_GET['p']) && is_numeric($_GET['p'])) { 
	...
	$q = "SELECT COUNT(account_id) FROM haller_shop_entity_account_information";
...
$q = "SELECT last_name, first_name, DATE_FORMAT(registration_date, '%M %d, %Y') AS dr, account_id FROM haller_shop_entity_account_information ORDER BY $order_by LIMIT $start, $display";		
$r = @mysqli_query ($conn, $q); 
//table for results
...
while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
	$bg = ($bg=='#eeeeee' ? '#ffffff' : '#eeeeee');
		echo '<tr bgcolor="' . $bg . '">
		<td align="left"><a href="edit_user.php?id=' . $row['account_id'] . '">Edit</a></td>
		<td align="left"><a href="delete_user.php?id=' . $row['account_id'] . '">Delete</a></td>
